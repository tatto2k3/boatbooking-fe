...
export default function Payment() {
    const [searchResult, setSearchResult, isLoading, setIsLoading, searchInfo,
        setSearchInfo, setTripType, airport, setAirport, departFlight, setDepartFlight, ariveFlight, setArriveFlight,
        ...
        setPassengerInfo, setSeatId, luggaeId, setLuggageId] = useSearch();
    ...
    const price = localStorage.getItem('priceOneWay');
    const boat_name = localStorage.getItem('boat_nameOneWay');
    const port_name = localStorage.getItem('port_nameOneWay');
    const fromLocation = localStorage.getItem('fromLocationOneWay');
    const toLocation = localStorage.getItem('toLocationOneWay');
    const departureDay = localStorage.getItem('departureDayOneWay');
    const departureTime = localStorage.getItem('departureTimeOneWay');
    const arrivalTime = localStorage.getItem('arrivalTimeOneWay');
    const scheduleID = localStorage.getItem('scheduleIDOneWay');
    const seatId = localStorage.getItem('seatId');

    const priceComeback = tripType == 'roundTrip' ? localStorage.getItem('priceComeback') : '';
    const boat_nameComeback = tripType == 'roundTrip' ? localStorage.getItem('boat_nameComeback') : '' ;
    const port_nameComeback = tripType == 'roundTrip' ? localStorage.getItem('port_nameComeback') : '' ;
    const fromLocationComeback = tripType == 'roundTrip' ? localStorage.getItem('fromLocationComeback') : '';
    const toLocationComeback = tripType == 'roundTrip' ? localStorage.getItem('toLocationComeback') : '' ;
    const departureDayComeback = tripType == 'roundTrip' ? localStorage.getItem('departureDayComeback'): '' ;
    const departureTimeComeback = tripType == 'roundTrip' ? localStorage.getItem('departureTimeComeback'): '' ;
    const arrivalTimeComeback = tripType == 'roundTrip' ? localStorage.getItem('arrivalTimeComeback'): '' ;
    const scheduleIDComeback = tripType == 'roundTrip' ? localStorage.getItem('scheduleIDComeback'): '' ;
    const seatIdComeback = tripType == 'roundTrip' ? localStorage.getItem('seatIdComeback'): '' ;
    ...
    const passport = localStorage.getItem('passengerInfo_passportNumber');
    ...
    let amount = tripType == 'oneWay' ? parseInt(price) : parseInt(price) + parseInt(priceComeback);
    console.log('price:', amount);


    const jsonData = {
        "ticket_amount": parseInt(amount),
        "customer_name": `${lastName}`,
        "customer_email": `${email}`,
        "customer_birthday": `${dateOfBirth}`,
        "customer_identify": `${passport}`,
        "customer_address": `${city}`,
        "seat_id": `${seatId}`,
        "flight_id": parseInt(scheduleID),
        "departure_day": `${departureDay}`,
        "arrive_day": `${departureDay}`,
        "departure_time": `${departureTime}`,
        "arrive_time": `${arrivalTime}`,
        "customer_phone": `${contact}`,
        "duration_time": `${formatTimeDuration(departureTime, arrivalTime)}`,
        "trip_type": `${tripType}`,
        "seat_idComeback": `${seatIdComeback}`,
        "flight_idComeback": parseInt(scheduleIDComeback),
        "departure_dayComeback": `${departureDayComeback}`,
        "arrive_dayComeback": `${departureDayComeback}`,
        "departure_timeComeback": `${departureTimeComeback}`,
        "arrive_timeComeback": `${arrivalTimeComeback}`,
        "duration_timeComeback": `${formatTimeDuration(departureTimeComeback, arrivalTimeComeback)}`,
    }
